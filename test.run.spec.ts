...
// ---------------------------------------------------------------------------
// TimeoutError under --wait emits partial stdout + exit 7
//
// When the --timeout polling deadline is exceeded (TimeoutError), the CLI
// must emit a partial {runId, status:"running"} JSON object to stdout BEFORE
// throwing, mirroring the existing RequestTimeoutError path so agents can
// recover the runId and chain into `testsprite test wait <runId>`.
// ---------------------------------------------------------------------------
...
describe('runTestRun --wait: TimeoutError writes partial JSON to stdout', () => {
  it('exit 7 AND stdout contains {runId, status:"running"} when --timeout polling deadline is exceeded', async () => {
    ...
    const base = Date.now();
    const realDateNow = Date.now;
    // After 6 Date.now calls the clock jumps past the 1s deadline (base + 2000 > base + 1000).
